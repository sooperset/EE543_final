...
class Learning():
    ...
    def correct_label(self, batch_pred1, batch_labels, t_is=0.1):
        ...
        with torch.no_grad():
            ...
            if t_is:
                mean_logp = logp1.max(1)[0]
                t_mean = mean_logp[batch_labels == 255].mean()
                t_max = mean_logp[batch_labels == 255].max()
                rate = (t_max - t_mean)
                t = t_mean + rate * t_is

                corrected = torch.where((batch_labels == 255) & (mean_logp > t), logp1.argmax(1), batch_labels)
            else:
                corrected = torch.where((batch_labels == 255), logp1.argmax(1), batch_labels)
